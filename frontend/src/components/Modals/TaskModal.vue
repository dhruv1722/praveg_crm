...
function redirect() {
	...
	} else if (doctype === "AI Leads") {
		routeName = "AI Lead";
		params = { aiLeadId: docname };
	} else if (doctype === "CRM Lead") {
		routeName = "Lead";
		params = { leadId: docname };
	}
...
function getOpenButtonLabel() {
	if (!props.task?.reference_doctype) return __("Open lead");

	const doctype = props.task.reference_doctype;
	if (doctype === "CRM Deal") {
		return __("Open deal");
	} else if (doctype === "AI Leads") {
		return __("Open AI lead");
	} else {
		return __("Open lead");
	}
}
