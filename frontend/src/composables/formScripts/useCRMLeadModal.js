...
export function useCRMLeadModal(lead) {
  ...
  function validateDates(showToast = false) {
    const { custom_check_in, custom_check_out } = lead.doc

    if (!custom_check_in || !custom_check_out) {
      lead.doc.custom_no_of_nights = 0
    }
    ...
    if (checkInDate && checkOutDate) {
      if (checkOutDate <= checkInDate) {
        ...
        lead.doc.custom_no_of_nights = diffDays
      }
    }
  ...
  function setupWatchers() {
    watch(
      () => [lead.doc.custom_hotel_property],
      () => {
        fetchExtraBedRate(lead.doc)
      },
      { immediate: true }
    )

    watch(
      () => [lead.doc.custom_check_in, lead.doc.custom_check_out],
      () => {
