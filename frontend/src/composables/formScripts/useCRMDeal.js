...
export function useCRMDeal(doc, document) {
  ...
  function hasAllPaymentReceiptsReceived() {
    const receipts = Array.isArray(doc.value?.custom_payment_receipt)
      ? doc.value.custom_payment_receipt
      : []

    return (
      receipts.length > 0 &&
      receipts.every(
        (row) => (row?.payment_receipt_status || '').trim() === 'Received'
      )
    )
  }
  ...
  function isEmailTemplateVisible(template) {
    ...
    const rule = emailTemplateVisibilityRules[templateName]
