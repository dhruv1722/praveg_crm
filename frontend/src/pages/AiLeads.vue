<template>
  <LayoutHeader>
    <template #left-header>
      <ViewBreadcrumbs v-model="viewControls" routeName="AI Leads" />
    </template>
    <template #right-header>
      <CustomActions
        v-if="AileadsListView?.customListActions"
        :actions="AileadsListView.customListActions"
      />
      <Button
        variant="solid"
        :label="__('Create')"
        iconLeft="plus"
        @click="showAiLeadModal = true"
      />
    </template>
  </LayoutHeader>
  <ViewControls
  ...
  <AiLeadsListView
    ref="AileadsListView"
    v-else-if="aileads.data && rows.length"
    v-model="aileads.data.page_length_count"
    v-model:list="aileads"
    :rows="rows"
    :columns="columns"
    :options="{
      showTooltip: false,
      resizeColumn: true,
      rowCount: aileads.data.row_count,
      totalCount: aileads.data.total_count,
    }"
    @loadMore="() => loadMore++"
  ...
  />
  <AiLeadModal
    v-if="showAiLeadModal"
    v-model="showAiLeadModal"
    :defaults="defaults"
  />
  <NoteModal
    v-if="showNoteModal"
...
import AiLeadModal from '@/components/Modals/AiLeadModal.vue'
...
import AiLeadsListView from '@/components/ListViews/AiLeadsListView.vue'
...
const AileadsListView = ref(null)
...
// Rows
const rows = computed(() => {
  if (!aileads.value?.data?.data) return []
  if (aileads.value.data.view_type === 'group_by') {
    if (!aileads.value?.data.group_by_field?.fieldname) return []
    return getGroupedByRows(
      aileads.value?.data.data,
      aileads.value?.data.group_by_field,
      aileads.value.data.columns,
    )
  } else if (aileads.value.data.view_type === 'kanban') {
    return getKanbanRows(aileads.value.data.data, aileads.value.data.fields)
  } else {
    return parseRows(aileads.value?.data.data, aileads.value.data.columns)
  }
})

const columns = computed(() => {
  let _columns = aileads.value?.data?.columns || []
...
function parseRows(rows, columns = []) {
  let view_type = aileads.value.data.view_type
  let key = view_type === 'kanban' ? 'fieldname' : 'key'
  let type = view_type === 'kanban' ? 'fieldtype' : 'type'

  return rows.map((lead) => {
    let _rows = {}
    aileads.value?.data.rows.forEach((row) => {
      _rows[row] = lead[row]
...
function onNewClick(column) {
  let column_field = aileads.value.params.column_field

  if (column_field) {
    defaults[column_field] = column.column.name
  }

  showAiLeadModal.value = true
}
  ...
  let actions = [
    {
      ...
      onClick: () => makeSmartFlowCallFromList(itemName, mobile_no),
      condition: () => !!mobile_no,
    },
    {
...
async function makeSmartFlowCallFromList(aiLead, mobile_no) {
  if (!mobile_no) {
